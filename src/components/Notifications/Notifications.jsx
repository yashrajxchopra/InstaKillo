...
export default function Notifications({setIsNotificatonsOpen}) {
  ...
  const fetchNotifications = async  ()=>{
    try {
      const token = localStorage.getItem("token");
      const { data } = await axios.get(`${API_URL}/api/getNotifications?page=${page}&limit=5`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
useEffect(() => {
  ...
  return (
    <div
      className='absolute top-14 right-4 w-64 p-4 bg-slate-50 dark:bg-darkgray shadow-lg rounded-lg z-50'
    >
      <h3 className='text-black dark:text-white font-bold mb-2'>Notifications</h3>
      <hr className="border-t border-gray-300 dark:border-gray-700 mb-2"/>
      <div className='flex flex-col gap-2 h-auto max-h-28'>
        {notifications?.length ===0 && <span className='text-black dark:text-white text-center'>No Notifications</span>}
        {notifications && notifications.map((notification, index) => (
          <span onClick={()=>handleClick(notification?.message?.trim().split(' ')[0], notification?.post)} key={notification._id}
          data-index={index} className='text-black dark:text-white cursor-pointer focus:underline'  ref={(el) => (notificationRefs.current[index] = el)} tabIndex={index}>{notification.message}</span>
        ))}
        {hasMore && <div ref={loader} className="h-2 bg-slate-50 dark:bg-darkgray" />}
      </div>
    </div>
  );
}
